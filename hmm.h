...
#define HMM_H
//joris
//#define MAXX_STATES 20
#define MAX_TRELLIS 200
...
class HMM {
	...
	bool diff_len;						// TRUE if strings are of different lengths
...
public:
	...
	int batch_test(char* filename);							// test using file
	double single_test(char* filename);						// test using file with one line  (Nam add)
	double test_one(int* string, int symbol_count);			// allocate space and then test one string 
