// file : hmm.cpp : implementation file
//
...
HMM::~HMM()
{
	...
	if (a_numer_sum_recur != NULL){
		delete[] a_numer_sum_recur;
	}
	if (a_numer_sum_next != NULL){
		delete[] a_numer_sum_next;
	}
	if (a_denom_sum_recur != NULL){
		delete[] a_denom_sum_recur;
	}
	if (a_denom_sum_next != NULL){
		delete[] a_denom_sum_next;
	}
	if (scaling_factors != NULL){
	...
	*/
	...
	while (prob_sum > min_delta_psum) {
		if (diff_len == true)  {
			...
			// reestimate routine initializes the prob_sum
			prob_sum = reestimate();
...
//
int HMM::batch_test(char *filename)
{
	int symbol_count;
...
double HMM::single_test(char *filename)
{
	int symbol_count;
	...
	val = test(strings[0], string_len[0]);
...
void HMM::dump_seq(ofstream &seq, int length)
{
	...
	for (j = 0; j < length; j++) {
		// fill in random
		prob_trans = double(rand()) / RAND_MAX;
		prob_out = double(rand()) / RAND_MAX;
...
void HMM::alloc_model_matrices() {
	//joris if pointers are not NULL, delete before new
...
void HMM::alloc_testing_matrices()
{
	...
	/*
	  ...
	  // scaling factors
	  delete [] scaling_factors;//joris
	  scaling_factors = new double[trellis_width];
	  */
	// joris
	// space for calculation tables
	// two dimensional array decls
	if (trellis_width <= MAX_TRELLIS) {
		if (alpha == NULL) {
			alpha = new double*[MAX_TRELLIS];
		...
		if (scaling_factors == NULL) {
			scaling_factors = new double[MAX_TRELLIS];
		}
	}
	else {
		delete[] alpha; //joris
...
void HMM::rnd_sym_probs(double prob_syms[])
{
	...
	for (int s = 0; s < max_symbols; s++) {
		prob_syms[s] = double(rand()) / RAND_MAX;
		prob_sum += prob_syms[s];
	}
	for (int s = 0; s < max_symbols; s++)  // normalize so add to one
...
// and returns the sum of the prob's difference 
//
...
// create space for testing strs
// and then test model for a single string.
//

double HMM::test_one(int* string, int symbol_count)
{
	double log_alpha;
...
// set/return the probability of generating a particular symbol 
...
// set/return the probability of making transition to the next state.
// (smooth with MIN_PROB)
//
...
void HMM::init_pointers()
{
	strings = NULL;
	string_len = NULL;		    		// use if strings are of different length
	States = NULL;                   	// matrix of transition and emission probs
...
void STATE::initpointers()
{
	recur_out = NULL; // array of output symbols
	next_out = NULL;  // array of output symbols
